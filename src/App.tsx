...
interface Note { id: string; title: string; content: string; time: number; isNew?: boolean; }
...
const App: React.FC = () => {
    ...
    const reloadNotes = useCallback(async () => {
        try {
            ...
            const dirs = files.filter(f => f.type === 'directory').map(d => d.name);
            setGroups(dirs);

            const loadFile = async (name: string, folder = '') => {
                ...
                return { id: folder ? `${folder}/${name}` : name, title: name, content: data as string, time: Date.now() }; // Simplified mtime
            };
            ...
            setNotes([...rootNotes, ...groupNotesArrays.flat()].sort((a, b) => b.time - a.time));
    ...
    const confirmDelete = async () => {
        if (!curId) return;
        try { await Filesystem.deleteFile({ path: `${DIR}/${curId}`, directory: Directory.Documents }); setNotes(n => n.filter(x => x.id !== curId)); setView('list'); setCurId(null); } catch (e) { }
            ...
            <div className={`view ${view === 'list' ? '' : 'view-hidden'}`}>
                <header>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <h1>{curGroup ? curGroup : (curGroup === '' ? t.uncategorized : t.allNotes)}</h1>
                    </div>
                    <button className="btn-icon" onClick={() => setView('settings')}><Icon d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z" /></button>
                </header>
                ...
                <div className="list-container" ref={listRef} onScroll={() => syncScroll('list')}>
                    {filteredNotes.length === 0 ? <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-dim)' }}>{t.noNotes}</div> :
                        filteredNotes.map(n => <div key={n.id} className="note-card" onClick={() => { setCurId(n.id); setView('editor'); }}><div className="note-title">{n.title}</div><div className="note-desc">{n.content.substring(0, 40) || '...'}</div><div className="note-time">{new Date(n.time).toLocaleString([], { hour: '2-digit', minute: '2-digit', year: 'numeric', month: '2-digit', day: '2-digit' })}</div></div>)}
                </div>
                    ...
                    <div className="toc-overlay" onClick={() => setSidebarOpen(false)}>
                        <div className="toc-sidebar" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="toc-list">
                                ...
                                {groups.map(g => <div key={g} className={`toc-item ${curGroup === g ? 'active' : ''}`} onClick={() => { setCurGroup(g); setSidebarOpen(false); }}>{g}</div>)}
                            </div>
            ...
            <div className={`view ${view === 'editor' ? '' : 'view-hidden'}`}>
                ...
                {moreOpen && (
                    <div className="more-menu-overlay" onClick={() => setMoreOpen(false)}><div className="more-menu" onClick={e => e.stopPropagation()}>
                        <div className="menu-item" onClick={() => { setRenameValue(curNote?.id || ''); setShowRename(true); setMoreOpen(false); }}><Icon d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7 M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" style={{ marginRight: 12 }} size={20} />{t.rename}</div>
                        <div className="menu-item" onClick={() => { setShowDeleteConfirm(true); setMoreOpen(false); }} style={{ color: '#ff4d4f' }}><Icon d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" style={{ marginRight: 12 }} size={20} />{t.deleteNote || t.delConfirm.split('？')[0]}</div>
                ...
                {showDeleteConfirm && <div className="modal-overlay" onClick={() => setShowDeleteConfirm(false)}><div className="modal-content" onClick={e => e.stopPropagation()}><h4>{t.delConfirm}</h4><div style={{ display: 'flex', gap: 10, marginTop: 10 }}><button className="modal-close" style={{ background: 'var(--surface)', color: 'var(--text)', flex: 1, marginTop: 0 }} onClick={() => setShowDeleteConfirm(false)}>{t.cancel}</button><button className="modal-close" style={{ background: '#ff4d4f', flex: 1, marginTop: 0 }} onClick={confirmDelete}>{t.ok}</button></div></div></div>}
            </div>
