...
internal object LanguageHelper {
    @JvmStatic fun getQuickQuoteHandler(language: Language) = try {
        language.quickQuoteHandler
    } catch (e: AbstractMethodError) { null }

    @JvmStatic fun getIndentAdvance(language: Language, content: ContentReference, line: Int, column: Int, spaceCountOnLine: Int, tabCountOnLine: Int) = try {
