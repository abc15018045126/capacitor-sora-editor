...
open class CodeEditor
    ...
    constructor(
        ...
        ContentListener,
        Formatter.FormatResultReceiver,
        ...
        fun <T : EditorBuiltinComponent> replaceComponent(
            clazz: Class<T>,
            replacement: T,
        ) {
        ...
        fun registerInlayHintRenderer(
            @NonNull renderer: InlayHintRenderer,
        ) {
            val oldValue = inlayHintRendererMap.put(renderer.typeName, renderer)
        ...
        public fun measureTextRegionOffset(): Float {
            return if (this.isLineNumberEnabled) {
                (
                    measureLineNumber() + dividerMarginLeft + dividerMarginRight + dividerWidth +
                        (if (renderer.hasSideHintIcons()) this.rowHeight else 0)
                )
            } else {
         ...
         */
        protected fun applyAttributeSets(
            attrs: AttributeSet?,
            defStyleAttr: Int,
            defStyleRes: Int,
        ) {
         ...
         */
        fun setSelectionHandleStyle(
            @NonNull style: SelectionHandleStyle?,
        ) {
        ...
        private fun setWordwrap(
            wordwrap: Boolean,
            antiWordBreaking: Boolean,
            supportRtlRow: Boolean,
        ) {
         ...
         * @see .setCursorAnimationEnabled
         */
        fun setCursorAnimator(
            @NonNull cursorAnimator: CursorAnimator,
        ) {
         ...
         */
        internal fun computeMatchedPositions(
            line: Int,
            positions: LongArrayList,
        ) {
            positions.clear()
        ...
        internal fun computeHighlightPositions(
            line: Int,
            positions: MutableLongLongMap,
        ) {
            positions.clear()
        ...
        internal fun createLayout(clearWordwrapCache: Boolean = true) {
            ...
            if (layout != null) {
                ...
                if (layout is WordwrapLayout && isWordwrap) {
                    val newLayout: WordwrapLayout =
                        WordwrapLayout(
                            this,
                            text,
                            this.isAntiWordBreaking,
                            this.isWordwrapRtlDisplaySupport,
                            layout as WordwrapLayout,
                            clearWordwrapCache,
                        )
        ...
        fun commitText(
            ...
            if (applySymbolCompletion && props!!.symbolPairAutoCompletion && text.length > 0) {
                ...
                pair =
                    languageSymbolPairs?.matchBestPair(
                        this, cursor!!.left(),
            ...
            if (pair != null && pair !== SymbolPairMatch.SymbolPair.EMPTY_SYMBOL_PAIR) {
                ...
                if (pair.shouldDoAutoSurround(editorText) && quoteHandler == null) {
                    ...
                    setSelectionRegion(
                        ...
                        cur.leftColumn,
                        cur.rightLine,
                        cur.rightColumn - pair.close.length,
                    )
            ...
            if (cur.isSelected()) {
                ...
                if (props!!.autoIndent && text.length !== 0 && applyAutoIndent) {
                    ...
                    if (first == '\n' || first == '\r') {
                        ...
                        try {
                            count +=
                                LanguageHelper.getIndentAdvance(
                                    editorLanguage!!,
         ...
         */
        internal fun setSelectionAround(
            line: Int,
            column: Int,
        ) {
            var column = column
        ...
        fun setDiagnosticIndicatorStyle(
                            ...
                            android.app.AlertDialog.Builder(context)
                                .setTitle(
                                    I18nConfig.getResourceId(
                                        if (replaceAll) R.string.sora_editor_replaceAll else R.string.sora_editor_replace,
                                    ),
                                )
                                .setView(et)
         ...
         */
        fun setLineNumberMarginLeft(
            @Px lineNumberMarginLeft: Float,
        ) {
        ...
        val lastVisibleRow: Int
            ...
            get() {
                ...
                return kotlin.math.max(
                    0,
                    kotlin.math.min(layout!!.rowCount - 1, layout!!.getRowIndexForY((this.offsetY + height).toFloat())),
                )
            }
            ...
            if (layoutBusy && !busy) {
                if (isWordwrap && touchHandler!!.positionNotApplied) {
                    ...
                    dispatchEvent(
                        ScrollEvent(
                            this,
                            scroller!!.getCurrX().toInt(),
                            scroller.getCurrY().toInt(),
                            0,
                            afterScrollY.toInt(),
         ...
         */
        fun moveOrExtendSelection(
            @NonNull movement: SelectionMovement,
            extend: Boolean,
        ) {
            if (extend) {
        ...
        protected fun copyTextToClipboard(
            ...
            try {
                val clip: CharSequence? =
                    if (text is Content) {
                        (text as Content).substring(start, end)
                    } else {
        ...
        fun setText(
            ...
            dispatchEvent(
                ContentChangeEvent(
                    ...
                    CharPosition(),
                    _text!!.indexer.getCharPosition(
                        this.lineCount - 1,
                        _text!!.getColumnCount(this.lineCount - 1),
                    ),
                    _text!!,
                    false,
                ),
        ...
        fun updateStyles(
            ...
            @Nullable range: StyleUpdateRange?,
        ) {
         ...
         */
        internal fun setExtracting(
            @Nullable request: ExtractedTextRequest?,
        ) {
            if (props!!.disallowSuggestions) {
         ...
         */
        internal fun extractText(
            @NonNull request: ExtractedTextRequest,
        ): ExtractedText? {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if ((
                    lastCursorState != cursorBlink?.visibility ?: false ||
                        !touchHandler!!.getScroller()
                            .isFinished
                ) && touchHandler!!.editorMagnifier.isShowing()
            ) {
                lastCursorState = cursorBlink?.visibility ?: false
        ...
        protected override fun onMeasure(
            ...
            if (MeasureSpec.getMode(widthMeasureSpec) !== MeasureSpec.EXACTLY ||
                ...
                val specs: Long =
                    ViewMeasureHelper.getDesiredSize(
                        widthMeasureSpec,
                        heightMeasureSpec,
                        measureTextRegionOffset(),
                        this.rowHeight.toFloat(),
                        isWordwrap,
                        tabWidth,
                        text,
                        renderer.paintGeneral,
                    )
         ...
         */
        public fun postDelayedInLifecycle(
            action: Runnable,
            delayMillis: Long,
        ): Boolean {
        ...
        override fun afterInsert(
            ...
            endLine: Int,
            endColumn: Int,
            @NonNull insertedContent: CharSequence,
        ) {
        ...
        override fun afterDelete(
            ...
            endLine: Int,
            endColumn: Int,
            @NonNull deletedContent: CharSequence,
        ) {
        ...
        override fun beforeModification(
            @NonNull content: Content,
        ) {
        ...
        override fun onFormatSucceed(
            @NonNull applyContent: CharSequence,
            @Nullable cursorRange: TextRange?,
        ) {
            postInLifecycle(
                Runnable {
                    val line: Int = cursor!!.leftLine
