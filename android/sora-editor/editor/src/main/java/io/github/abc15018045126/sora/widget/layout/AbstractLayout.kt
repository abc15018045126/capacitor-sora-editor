...
abstract class AbstractLayout(@JvmField protected var editor: CodeEditor?, @JvmField protected var text: Content?) : Layout {
    ...
    override fun destroyLayout() { editor = null; text = null }
    ...
    protected open class TaskMonitor(private val taskCount: Int, private val callback: Callback) {
        private val results: Array<Any?> = arrayOfNulls(taskCount); private var completedCount = 0; private var cancelledCount = 0
        @Synchronized fun reportCompleted(result: Any?) { results[completedCount++] = result; if (completedCount == taskCount) callback.onCompleted(results, cancelledCount) }
        @Synchronized fun reportCancelled() { cancelledCount++; reportCompleted(null) }
        interface Callback { fun onCompleted(results: Array<Any?>, cancelledCount: Int) }
    }
    ...
    protected abstract inner class LayoutTask<T>(private val monitor: TaskMonitor) : Runnable {
        ...
        override fun run() = if (shouldRun()) monitor.reportCompleted(compute()) else monitor.reportCancelled()
        protected abstract fun compute(): T
    }

    companion object {
        protected const val SUBTASK_COUNT = 8; protected const val MIN_LINE_COUNT_FOR_SUBTASK = 3000
        private val executor = ThreadPoolExecutor(2, max(2, Runtime.getRuntime().availableProcessors()), 1, TimeUnit.MINUTES, LinkedBlockingQueue(128))
    }
}
