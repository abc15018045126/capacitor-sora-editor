 ...
 * @author Akash Yadav
 */
class EditorKeyEventHandler internal constructor(private val editor: CodeEditor) {

    internal val keyMetaStates: KeyMetaStates = KeyMetaStates(editor)
     ...
     * [EditorKeyEventHandler.keyMetaStates] must be notified about the key event before this
     * method is called.
     *
    ...
    fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        ...
        val result = handleKeyEvent(event, editorKeyEvent, keybindingEvent, keyCode, isShiftPressed, isAltPressed, isCtrlPressed)
        if (result != null) {
            ...
            KeyEvent.KEYCODE_DPAD_DOWN -> {
                if (isCtrlPressed) {
                    if (isShiftPressed) {
                        ...
                        // Update selection
                        val indexer = editorText.indexer
                        val newLeft = indexer.getCharPosition(left.line + 1, left.column)
                        val newRight = indexer.getCharPosition(right.line + 1, right.column)
                        if (left.index != right.index) {
                            ...
                            if (backupAnchor != null) {
                                editor.selectionAnchor = if (backupAnchor == left) newLeft else newRight
                            }
                        } else {
                    ...
                    editor.eventHandler.scrollBy(0f, editor.rowHeight.toFloat())
                    return true
                }
            ...
            KeyEvent.KEYCODE_DPAD_UP -> {
                if (isCtrlPressed) {
                    if (isShiftPressed) {
                        ...
                        editorText.insert(right.line - 1, editorText.getColumnCount(right.line - 1), editor.lineSeparator.content + prev)
                        editorText.endBatchEdit()
    ...
    private fun handlePrintingKey(event: KeyEvent, keyCode: Int): Boolean {
        ...
        if (charCode != 0 && editor.isEditable) {
            if (charCode == KeyCharacterMap.HEX_INPUT.code || charCode == KeyCharacterMap.PICKER_DIALOG_INPUT.code) {
            ...
            if (editorCursor.left + 1 == editorCursor.right) {
                ...
                val composed = KeyCharacterMap.getDeadChar(base.code, charCode)
                if (composed != base.code && event.repeatCount == 0) {
                    charCode = composed
                    dead = false
                }
            }
            ...
            if (dead) {
                if (!editor.isTextSelected || (editorCursor.left + 1 == editorCursor.right && editorText[editorCursor.left].code == charCode)) {
                    editor.setSelection(editorCursor.rightLine, editorCursor.rightColumn, SelectionChangeEvent.CAUSE_DEAD_KEYS)
                    editor.commitText(String(Character.toChars(charCode)))
                    val charCount = Character.charCount(Character.codePointBefore(editor.text.getLine(editorCursor.rightLine), editorCursor.rightColumn))
                    editor.setSelectionRegion(editorCursor.rightLine, editorCursor.rightColumn - charCount, editorCursor.rightLine, editorCursor.rightColumn, SelectionChangeEvent.CAUSE_DEAD_KEYS)
                }
                return true
            }
                ...
                for (handler in handlers) {
                    if (handler != null) {
                        if (handler.matchesRequirement(editorText, editorCursor.left(), editor.styles)) {
                            try {
                                val result = handler.handleNewline(editorText, editorCursor.left(), editor.styles, editor.tabWidth)
                                editor.commitText(result.text, false)
                                val delta = result.shiftLeft
                                if (delta != 0) {
                                    val newSel = max(editorCursor.left - delta, 0)
                                    val charPosition = editorCursor.indexer.getCharPosition(newSel)
