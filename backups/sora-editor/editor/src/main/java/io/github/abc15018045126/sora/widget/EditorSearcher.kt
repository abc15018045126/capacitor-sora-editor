 ...
 */
class EditorSearcher internal constructor(private val editor: CodeEditor) {
    ...
    private var cyclicJumping = true

    init {
        this.editor.subscribeEvent<ContentChangeEvent> { _, _ ->
            if (hasQuery()) {
                executeMatch()
            }
        }
    ...
    /**
     * Jump cyclically when calling [gotoNext] and [gotoPrevious]
     * @see isCyclicJumping
     */
    fun setCyclicJumping(cyclicJumping: Boolean) {
        this.cyclicJumping = cyclicJumping
    }

    /**
     * @see setCyclicJumping
     */
    fun isCyclicJumping(): Boolean {
    ...
    private fun executeMatch() {
        ...
        val options = searchOptions ?: return
     ...
     */
    fun getCurrentMatchedPositionIndex(): Int {
        checkState()
     ...
     * @throws IllegalStateException if no search is in progress
     */
    fun getMatchedPositionCount(): Int {
        checkState()
        if (!isResultValid) {
            return 0
        }
        ...
        Thread {
            try {
                ...
                if (options.type == SearchOptions.TYPE_REGULAR_EXPRESSION && options.regexBackrefGrammar != null) {
                    ...
                    var tokens: List<io.github.abc15018045126.sora.util.regex.RegexBackrefToken>? = null
                    var delta = 0
                    val textStr = sb.toString()
                    for (i in 0 until res.size()) {
                        val region = res.get(i)
                        val start = IntPair.getFirst(region)
                        val end = IntPair.getSecond(region)
                        val regionText = textStr.substring(start, end)
                        if (matcher == null) {
                        ...
                        if (tokens == null) {
                            tokens = RegexBackrefParser(options.regexBackrefGrammar).parse(
                                replacement,
                                matcher.groupCount()
                            )
                        }
    ...
    protected val isResultValid: Boolean
        get() = currentThread == null || !currentThread!!.isAlive

    /**
     * Search options for [EditorSearcher.search]
     */
    class SearchOptions @JvmOverloads constructor(
        @IntRange(from = 1, to = 3) val type: Int,
     ...
     */
    private inner class SearchRunnable(content: Content, private val options: SearchOptions, private val pattern: String) :
        Runnable {
